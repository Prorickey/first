...
export class FirstFTCAPI {
     ...
     * @throws {Error} If the token is not provided.
     */
    constructor(private token: string, private season: Season = Season.IntoTheDeep) {
     ...
     */
    async getIndex() {
        const response = await fetch(`${CONSTANTS.ftc.baseURL}/${CONSTANTS.ftc.version}`, {
            method: 'GET',
            headers: {
     ...
     */
    async getTeams(options?: { teamNumber?: number; eventCode?: string; state?: string }) {
        ...
        if (options?.teamNumber && (options.eventCode || options.state)) {
            throw new Error("Cannot specify both teamNumber and (eventCode or state) in the same request");
        }
    ...
    async getEvents(options?: { eventCode?: string; teamNumber?: number }) {
        ...
        if (options?.eventCode && options?.teamNumber) {
            throw new Error("Cannot specify both eventCode and teamNumber in the same request");
        }
     ...
     */
    async getMatches(eventCode: string, options?: { 
        ...
        if (options?.teamNumber && options?.matchNumber) {
            throw new Error("Cannot specify both teamNumber and matchNumber in the same request");
        }
        
        // You need to specify tournamentLevel if you specify matchNumber, start, or end
        if ((options?.matchNumber || options?.start || options?.end) && !options?.tournamentLevel) {
            throw new Error("Must specify tournamentLevel when using matchNumber, start, or end");
        }
     ...
     */
    async getRankings(eventCode: string, options?: { teamNumber?: number; top?: number }) {
        if (!eventCode) {
            throw new Error("Event code is required");
        }
        ...
        if (options?.teamNumber && options?.top) {
            throw new Error("Cannot specify both teamNumber and top in the same request");
        }
    ...
    async getAwards(options: { eventCode?: string; teamNumber?: number }) {
        if (!options.eventCode && !options.teamNumber) {
            throw new Error("Must specify at least one of eventCode or teamNumber");
        }
     ...
     */
    async getScores(eventCode: string, tournamentLevel: string, options?: { 
        teamNumber?: number; 
        matchNumber?: number;
        start?: number;
        end?: number;
     ...
     */
    async getSchedule(eventCode: string, options: { 
        tournamentLevel: string;
        teamNumber?: number;
    }) {
        if (!eventCode) {
     ...
     */
    async getHybridSchedule(eventCode: string, tournamentLevel: string, options?: { 
        start?: number;
        end?: number;
    }) {
        if (!eventCode) {
